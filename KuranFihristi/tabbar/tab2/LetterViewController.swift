...
class LetterViewController: UITableViewController, UISearchResultsUpdating, UISearchBarDelegate {
     ...
     func filter(searchText:String){
         if !searchText.isEmpty {
             ...
             letters = fullLetters.filter { (Letter) -> Bool in
                 ...
                 return "\(Letter.letterId).".contains(searchText) || itemText.contains(loverSearch) || itemText.contains(textAz)
             }
     ...
     override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
         ...
         if letterItem.letterId == 0 {
             ...
             let cell = tableView.dequeueReusableCell(withIdentifier: "letterViewCell", for: indexPath as IndexPath) as! LetterViewCell
             cell.letterNameLabel.text = " \(letterItem.letterName)"
             return cell
         }
     }
